...
def task_delete(task_id):
	...
	record_delete_task = Task.query.filter_by(task_id=task_id).first()
	original_project_id = record_delete_task.project.project_id
	db.session.delete(record_delete_task)
	db.session.commit()
...
def project_delete(project_id):
	...
	record_delete_project = Project.query.filter_by(project_id=project_id).first()
	db.session.delete(record_delete_project)
	db.session.commit()
	flash('Project deleted successfully','green')
	return redirect(url_for('show_projects'))		
